...
$.formValidator = 
{
	...
	initConfig : function(controlOptions)
	{
		...
		if(settings.formid!=""){						
			var resFn = function(){													
				var orPath = $.formValidator.pageIsValid(settings.validatorgroup);
				if(orPath){
					if(settings.onOkCBFun!=undefined){
						settings.onOkCBFun();
					}else if(settings.onOkCBFunParam!=undefined){
						settings.onOkCBFunParam(settings.formid);
					}
				}//如果通过了				
				return orPath;	
				};
			if(settings.buttonId!=""&&settings.buttonId!=undefined){						
				$("#"+settings.buttonId).click(resFn);							
			}else{
				if(settings.actEveObj!=undefined)
				{				
					settings.actEveObj.bind("click",resFn);							
				}
				else
				{
					$("#"+settings.formid).submit(resFn);	
				}
			}
		};
	...
	pageIsValid : function (validatorgroup)
	{
		...
		else
		{
			...
			/*目的是为了给输入框定焦。但是想上传组件它没有输入框，无法定焦，会有异常。所以不做定焦处理了
	...
	regexValid : function(returnObj)
	{
		...
		if(elem.settings[0].empty && elem.value==""){
			...
			//zhouqing 2013-10-30  regexpress=="^\\S+"非空验证
			if($("#"+id).val()!="" || regexpress=="^\\S+"){
				setting.isvalid = (new RegExp(regexpress, setting.param)).test($.trim($("#"+id).val()));
			}else{
				setting.isvalid = true;
				return;
			}
...
$.fn.formValidator = function(cs) 
{
	...
	//获取该校验组的全局配置信息
	cs = cs || {};
	if(cs.validatorgroup == undefined){cs.validatorgroup = setting.validatorgroup;}
